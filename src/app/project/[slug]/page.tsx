...
import { isHtml, plainToHtml, sanitizeHtml, stripHtml } from "@/lib/richText";
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const description = stripHtml(project.description).substring(0, 160);
...
export default async function ProjectPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const jsonLd = {
    ...
    description: stripHtml(project.description).substring(0, 160),
  ...
  return (
    <article className="max-w-4xl mx-auto px-6 py-16 page-enter">
      ...
      <Reveal delay={0} className="prose prose-lg max-w-none mb-16">
        ...
        <div
          className="rich-text text-ink-light leading-relaxed"
          dangerouslySetInnerHTML={{
            __html: sanitizeHtml(isHtml(project.description) ? project.description : plainToHtml(project.description)),
          }}
        />
      </Reveal>
